...
class BankingRiskModel:
    ...
    def create_banking_features(self, df):
        ...
        df['spending_stability'] = 1 / (1 + df['spending_var_6m'])  # Inverso de varianza
        df['spending_to_income_ratio'] = df['spending_monthly'] / df['income_monthly']
        df['savings_rate'] = (df['income_monthly'] - df['spending_monthly']) / df['income_monthly']
        ...
        df['debt_service_ratio'] = df['current_debt'] / df['income_monthly']
    ...
    def calculate_age_risk(self, age):
        ...
        optimal_age = 35
        risk = np.abs(age - optimal_age) / optimal_age
        return risk
    ...
    def train_advanced_model(self, X_train, y_train, X_valid, y_valid):
        ...
        # Hiperparámetros optimizados para modelo bancario
        params = {
            'objective': 'binary',
            'metric': 'auc',
            'boosting_type': 'gbdt',
            'num_leaves': 31,  # Aumentado para más complejidad
            'learning_rate': 0.05,
            'n_estimators': 3000,  # Aumentado
            'subsample': 0.8,
            'subsample_freq': 5,
    ...
    def evaluate_model(self, X_train, y_train, X_valid, y_valid, X_test, y_test):
        ...
        for name, (X, y) in datasets.items():
            ...
            print(f"   {name}: AUC={metrics['auc_roc']:.3f}, Acc={metrics['accuracy']:.3f}")
